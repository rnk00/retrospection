...
### 1-2. RefreshToken (로그인 유지)

#### 1. Access Token과 저장 방식이 다른 이유

Access Token(JWT)은 서명만 검증하면 유효성을 확인할 수 있는 자기 완결적 토큰이라 DB에 저장할 필요가 없다. 게다가 API 요청마다 매번 검증해야 해서, DB에 저장하지 않는 편이 성능상 유리하다.

반면 로그아웃 시 토큰을 진짜로 무효화할 수 있어야 한다는 요구사항이 있었다. 서명 검증 방식만으로는 "이 토큰은 이제 무효"라고 선언할 방법이 없어서, Refresh Token은 DB에 저장하고 로그아웃 시 해당 행을 삭제하는 방식으로 관리하기로 하였다. Refresh Token은 Access Token이 만료됐을 때만 가끔 검증하면 되므로, DB 조회 비용을 감당할 만하다고 판단하였다.

#### 2. 기기별 다중 로그인 지원

한 유저가 여러 기기(폰, PC 등)에서 동시에 로그인하는 경우를 고려하였다.

* `user_id`에 UNIQUE 제약을 걸어 유저당 하나만 유지 — 새로 로그인하면 이전 기기의 로그인이 끊김
* `user_id`에 제약 없이 여러 개 허용 — 기기별로 독립적인 로그인 유지

기기 하나에서 로그인해도 다른 기기 세션이 끊기지 않는 게 일반적인 사용자 기대에 맞아서, **후자(제약 없음, `User : RefreshToken = 1:N`)로 결정**하였다.

#### 3. token 컬럼 제약

갱신 요청이 오면 그 토큰 값으로 DB에서 빠르고 정확하게 조회해야 하므로 `UNIQUE` 제약을 적용하였다.

### 변경 사항 요약

* Access Token은 DB 저장이 필요 없지만, 로그아웃 시 무효화가 가능해야 하는 Refresh Token은 별도 테이블로 관리하기로 하였다.
* 기기별 독립적인 로그인 유지를 위해 `User : RefreshToken = 1:N`으로 설계하였다 (`user_id`에 UNIQUE 없음).
* 토큰 값으로 빠르게 조회하기 위해 `token`에 `UNIQUE` 제약을 적용하였다.

---

### 1-3. Retrospective, KptItem
...
GitHub 업로드 후 KPT를 수정하면 DB와 GitHub의 파일 내용이 달라지는데, 이를 화면에 표시할 방법이 필요하였다.

`updated_at`으로 판단하는 방법을 고려하였으나, KPT 내용은 `Retrospective`가 아니라 `KptItem`에 있어서 `KptItem`만 수정된 경우 `Retrospective.updated_at`은 변하지 않는다는 문제가 있었다.

따라서 `Retrospective`에 `is_github_synced` boolean 컬럼을 추가하여 직접 관리하기로 하였다.

* GitHub 업로드 성공 시 `true`로 변경한다.
* 점수(score) 변경, KPT 항목(생성/수정/삭제/순서변경) 중 하나라도 발생하면 `false`로 되돌린다.

이 방식은 타임스탬프 비교가 필요 없이 단순하지만, 관련된 모든 수정 API에서 빠짐없이 `false` 처리를 해줘야 한다는 점에 유의해야 한다.
...
* GitHub 동기화 여부 표시를 위해 `is_github_synced` 컬럼을 추가하였다.
...
erDiagram
    ...
    RETROSPECTIVE {
        ...
        boolean is_github_synced "NOT NULL, default false"
        datetime created_at
        datetime updated_at
    }
...
**제약조건**: `USER`에 `UNIQUE(provider, provider_user_id)`. `REFRESH_TOKEN`에 `UNIQUE(token)` (`user_id`는 제약 없음, 기기별 다중 로그인 지원). `RETROSPECTIVE`에 `UNIQUE(user_id, date)`. `KPT_ITEM`은 `type`별 최대 20개(서버 검증, DB 제약 아님).
...
분석 화면을 다시 설계하면서, 추천 행동(Try)이 하나의 텍스트가 아니라 **여러 개의 새로운 추천**으로 제공되어야 한다는 요구사항이 추가되었다 (기존 Try를 반복하지 않는 새로운 실천 방안 여러 개, 화면에 번호는 안 붙이지만 리스트 형태).

`try` 컬럼 하나로는 여러 개의 추천을 저장할 수 없으므로, `AiRecommendation`/`KptItem`과 같은 이유로 `ProblemPattern : ProblemPatternTry = 1:N` 구조로 분리하였다.

`problem`(반복된 문제)은 여전히 하나의 텍스트로 유지한다 — "많이 언급된 Problem TOP 3"는 이것과 별개의, 아직 설계 전인 기능이다.
...
* Try가 여러 개의 추천을 제공해야 해서 `try` 컬럼을 제거하고 `ProblemPatternTry` 테이블로 분리하였다.
...
erDiagram
    ...
    PROBLEM_PATTERN ||--o{ PROBLEM_PATTERN_TRY : suggests
