...
**최종 결정 (KPT를 텍스트 컬럼에서 KptItem 테이블로 분리)**: 상세 기획을 다시 짜면서, Keep/Problem/Try 각각이 최대 20개까지 항목을 가질 수 있고, 드래그로 순서를 바꿀 수 있어야 한다는 요구사항이 나옴. 텍스트 컬럼 하나로는 "여러 개의, 순서가 있는, 개별 수정 가능한 항목"을 표현할 수 없어서 (Tag/AiRecommendation을 분리했던 것과 같은 이유), `Retrospective : KptItem = 1:N`으로 분리함.
...
- `content`가 빈 값인 항목은 저장하지 않기로 함 (화면엔 빈 입력창이 있어도 DB엔 안 남음).
...
이 테이블은 **홈 화면 캘린더 상단에 뜨는 "이번 주 요약"** 전용이라 기간이 항상 고정(월요일 시작)이고, 재생성하면 기존 값을 덮어씀(`UNIQUE(user_id, week_start)`). 사용자가 기간을 자유롭게 골라보는 요약은 §2-4-1 Summary로 별도 분리함.

**최종 결정 (과거 주 넘겨보기 추가)**: 처음엔 "이번 주"만 보여줬는데, 이러면 월요일엔 그 주 데이터가 없어서 요약이 텅 비고, 지난 주들을 돌아볼 방법도 없었음. 롤링 7일 윈도우로 바꿔봤다가(캐싱을 날짜 단위로 관리해야 함) — 그러면 "주 단위로 어떻게 지냈는지" 비교가 애매해져서 되돌림. 대신 **"이번 주" 고정은 유지하되, 캘린더 화면에서 지난 주들을 넘겨볼 수 있는 기능**을 추가하기로 함. 넘겨보다가 만난 주가 아직 한 번도 생성된 적 없으면(그 주가 "이번 주"였을 때 방문하지 않아 자동 생성이 안 된 경우), 그 자리에서 생성해서 저장함 — "이번 주"에 적용하던 "없으면 생성, 있으면 캐시" 정책을 넘겨보기 대상 주에도 동일하게 적용하는 것뿐이라 별도 컬럼/테이블 변경은 필요 없음(주 단위로 이미 각각 행이 남아있어서, 넘겨보기 자체가 곧 히스토리 조회임). 미래 주는 데이터가 없으므로 넘겨보기 대상에서 제외.
...
### 2-7. KptNote (캘린더 화면 전용, 신규)

날짜/회고에 묶이지 않고 독립적으로 쌓이는 Keep/Problem/Try 메모. 캘린더 화면과 분석 탭 양쪽에서 추가할 수 있지만 같은 테이블/API를 공유함. AI는 관여하지 않는 순수 사용자 메모라 §2-4 ProblemPattern(AI 분석)과는 완전히 별개 기능.
