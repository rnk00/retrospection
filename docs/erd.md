...
**추가 결정 (GitHub 연동 설정 컬럼)**: 원래는 `repo_owner`, `repo_name`, `auto_commit_enabled`를 별도 `GithubIntegration` 테이블(§2-5)로 분리하려 했으나, User와 1:1이고 컬럼도 3개뿐이라 분리해도 실익이 없다고 판단해 User에 직접 컬럼으로 추가하기로 정정함. 이유는 §2-5 참고. `access_token`은 그대로 OAuthAccount(provider='GITHUB')를 참조.
...
AI 추천, 반복 문제 분석, 주간 요약. (Tag는 제외, GitHub 연동 설정은 §1-1 User로 통합 — 아래 참고)
...
access_token도 GithubIntegration에 따로 저장하는 안을 먼저 생각했는데(매번 OAuthAccount에서 가져오는 게 번거롭다는 이유), 토큰이 만료돼서 갱신할 때 두 테이블을 다 고쳐야 하고 하나만 놓치면 오래된 토큰으로 API를 호출하게 되는 문제가 있음을 확인함. → OAuthAccount(provider='GITHUB')의 토큰을 그대로 참조하는 걸로 정리. (이 부분은 그대로 유지)

**최종 결정: 별도 테이블 제외, User에 컬럼으로 통합.** 애초에 분리했던 이유는 "정규화 원칙상 컬럼이 null로 남는 게 깔끔하지 않아서"였지, "합치면 실제로 문제가 생겨서"가 아니었음. 막상 따져보면: (1) User와 1:1 관계라 부모 테이블에 얹어도 데이터 중복/정합성 문제가 없음 — 1:N이었다면 얘기가 다름, (2) 컬럼이 3개뿐이라 null 비용이 무시할 만한 수준, (3) 다중 레포 계획도 없어서 분리해봐야 이점이 없음. → `repo_owner`, `repo_name`, `auto_commit_enabled`를 별도 GITHUB_INTEGRATION 테이블 대신 USER 테이블에 직접 추가 (§1-1 다이어그램 참고). access_token 참조 방식은 변경 없음.
